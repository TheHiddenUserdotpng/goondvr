...
func SetupViews(r *gin.Engine) {
	...
	r.POST("/edit_channel/:channelID", EditChannel)
	...
	r.POST("/skip_current_stream/:channelID", SkipCurrentStream)
	r.POST("/clip_channel/:channelID", ClipChannel)
	r.GET("/api/recordings/:channelID", ListRecordings)
	r.POST("/api/manual_upload/:channelID", ManualUploadRecording)
	r.POST("/api/manual_upload_all", ManualUploadAll)
	r.GET("/api/upload_queue", GetUploadQueue)
	r.GET("/api/recording_file/:channelID", RecordingFile)
	r.GET("/api/recording_thumb/:channelID", RecordingThumb)
	r.GET("/api/clips/:channelID", ListClips)
	r.GET("/api/clip_file/:channelID", ClipFile)
	r.POST("/api/clip_recording/:channelID", ClipFromRecording)
